...
public:
    weak_wolfe(data_t c1=1e-4, data_t a0=1, data_t a1=0, bool version = 0){
        successCheck(((c1>0) && (c1<1)),"The constant c1 must be in (0,1).\n");
        _c1 = c1;
        _a0 = a0;
        _a1 = a1;
        _version = version;
...
public:
    regular_wolfe(data_t c1=1e-4, data_t c2=0.9, data_t a0=1, data_t a1=0, data_t step_max=1e6, bool version = 0){
        successCheck(((c1>0) && (c1<1) && (c2>c1) && (c2<1)),"The constants c1, c2 must satisfy 0<c1<c2<1\n");
        _c1 = c1;
        _c2 = c2;
        _a0 = a0;
        _a1 = a1;
...
public:
    ...
    bfgs(const int niter, const int max_trial, const data_t threshold=0, lsearch * ls=nullptr, std::shared_ptr<vecReg<data_t> > H0=nullptr):nlsolver(niter,max_trial,threshold,ls){
        if (H0 != nullptr) {
            successCheck(((H0->getHyper()->getNdim()== 2) && (H0->getHyper()->getAxis(1).n == H0->getHyper()->getAxis(2).n)),"Initial inverse Hessian must be a square matrix.\n");
            _H = new matrix(H0);
            successCheck(testSPD(_H),"The provided initial Hessian matrix must be SPD\n");
        }
        else _H=nullptr;
    }
...
public:
    ...
    lbfgs(const int niter, const int max_trial, const data_t threshold=0, lsearch * ls=nullptr, std::shared_ptr<vecReg<data_t> > H0=nullptr, int m=5):nlsolver(niter,max_trial,threshold,ls){
        if (H0 != nullptr) {
            successCheck((H0->min() > 0),"Initial diagonal inverse Hessian must be definite positive.\n");
            _H0 = H0->clone();
        }
        else _H0=nullptr; 
        _m = m;
