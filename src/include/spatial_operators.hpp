...
void esat_scale_boundaries(data_t** in, int nx, int ny, int nz, data_t dx, data_t dy, data_t dz, int ixmin, int ixmax, int iymin, int iymax, int izmin, int izmax, const data_t** par, data_t dt, bool top, bool bottom, bool left, bool right, bool front, bool back){
    ...
    // for VTI it must also contain c13, eps and gamma (although only eps will be used)
    data_t h0 = 17.0/48;
    ...
    if (top){
        ...
        if (izmin==0){
            ...
            if (left){
                ...
                #pragma omp parallel for private(scalerx,scalery,scalerz)
                for (int iy = std::max(1,iymin); iy < std::min(ny-1,iymax); iy++){
                    scalerx = sqrt(par[1][IXYZ(ix,iy,iz)]/par[2][IXYZ(ix,iy,iz)])*dt / (2 * dz * h0) + sqrt(f(par,IXYZ(ix,iy,iz))*(par[0][IXYZ(ix,iy,iz)]+2*par[1][IXYZ(ix,iy,iz)])/par[2][IXYZ(ix,iy,iz)])*dt / (2 * dx * h0);
            ...
            if (front){
                ...
                for (int ix = std::max(1,ixmin); ix < std::min(nx-1,ixmax); ix++){
                    ...
                    scalery = sqrt(par[1][IXYZ(ix,iy,iz)]/par[2][IXYZ(ix,iy,iz)])*dt / (2 * dz * h0) + sqrt(f(par,IXYZ(ix,iy,iz))*(par[0][IXYZ(ix,iy,iz)]+2*par[1][IXYZ(ix,iy,iz)])/par[2][IXYZ(ix,iy,iz)])*dt / (2 * dy * h0);
        ...
        if (ixmin==0){
            ...
            for (int iy = std::max(1,iymin); iy < std::min(ny-1,iymax); iy++){
                for (int iz = std::max(1,izmin); iz < std::min(nz-1,izmax); iz++){
                    ...
                    scalerz = scalery;
                    scalerx = sqrt(f(par,IXYZ(ix,iy,iz))*(par[0][IXYZ(ix,iy,iz)]+2*par[1][IXYZ(ix,iy,iz)])/par[2][IXYZ(ix,iy,iz)])*dt / (2 * dx * h0);
                    in[0][IXYZ(ix,iy,iz)] /= (1+scalerx);
                    in[1][IXYZ(ix,iy,iz)] /= (1+scalery);
                    in[2][IXYZ(ix,iy,iz)] /= (1+scalerz);
                }
        ...
        if (iymin==0){
            ...
            for (int ix = std::max(1,ixmin); ix < std::min(nx-1,ixmax); ix++){
                for (int iz = std::max(1,izmin); iz < std::min(nz-1,izmax); iz++){
                    ...
                    scalerz = scalerx;
                    scalery = sqrt(f(par,IXYZ(ix,iy,iz))*(par[0][IXYZ(ix,iy,iz)]+2*par[1][IXYZ(ix,iy,iz)])/par[2][IXYZ(ix,iy,iz)])*dt / (2 * dy * h0);
                    in[0][IXYZ(ix,iy,iz)] /= (1+scalerx);
                    in[1][IXYZ(ix,iy,iz)] /= (1+scalery);
                    in[2][IXYZ(ix,iy,iz)] /= (1+scalerz);
                }
