...
#include "vecReg.hpp"
#define successCheck(ans,msg) { _successCheck((ans), __FILE__, __LINE__,(msg)); }

inline void _successCheck(bool success, std::string file, int line, std::string msg){
    if (!success) {
...
void ttnormalize(data_t * __restrict d, data_t * norms, int nt, int ntr, data_t zero=1e-16);
