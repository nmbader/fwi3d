...
int rank=0;
    ...
    if (_H == nullptr) _H = new matrix(n,1);
        ...
        successCheck(sqrt(_H->getMat()->getN123())==n,"The provided inverse Hessian has an incorrect size\n");
    }
...
void bfgs::updateH(matrix * H ,std::shared_ptr<vecReg<data_t> > s, std::shared_ptr<vecReg<data_t> > y){
    ...
    successCheck(rho > 0,"The value of rho=1/(s'y) must be > 0 when updating the Hessian matrix\n");
    data_t a = y->dot(x);
    data_t ui;
...
int rank=0;
    ...
    else successCheck(_H0->getN123()==n,"The provided inverse Hessian has an incorrect size\n");
