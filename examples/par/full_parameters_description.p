...
# source: single source time function with a single time axis or 2D/3D time function with time, component (for vector or moment tensor sources), and source index axes
# model: 4D model with depth, X, Y, and parameter axes
# data: 2D/3D data with Time, receiver-source concatenated, and component axes
# output
# wavefield: 4D/5D with Depth, X, Y, component (if many), and Time axes 
...
# first DAS channel dip (in radians from x-axis clockwise), azimuth (in radians from x-axis anti-clockwise when looked from above), and increments
rdip0=1.5708
raz0=0.0
rdipinc=0.0
razinc=0.0
...
# azimuth of the vector force (in radians from x-axis anti-clockwise when looked from above).
fazimuth=0.0
...
# compression rate (integer) when ZFP is enabled
compression_rate=0